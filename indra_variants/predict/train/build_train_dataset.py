...
X_FILE = os.path.join(DATA_PATH, "training_feature_esm2.tsv")
PATH_FILE = os.path.join(DATA_PATH, "variant_paths.tsv")
EMB_FILE = os.path.join("node_embeds.pt")
LABEL_CLASSIFIED_FILE = os.path.join(DATA_PATH, "label_classified.tsv")
OUT_FILE = os.path.join(DATA_PATH, "path_dataset_bag_full.pt")
W_VAR_PT = os.path.join(DATA_PATH, "W_var.pt")
...
if __name__ == '__main__':
    ...
    if os.path.exists(W_VAR_PT):
        W_var = torch.load(W_VAR_PT);
        print("Loaded W_var")
    else:
        W_var = torch.randn(D_VAR, D_TOK) / np.sqrt(D_VAR)
        torch.save(W_var, W_VAR_PT);
        print("Initialised W_var randomly")
